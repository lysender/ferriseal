...
use crate::Result;
use crate::error::{
    DbInteractSnafu, DbPoolSnafu, DbQuerySnafu, MaxBucketsReachedSnafu, MaxVaultsReachedSnafu,
    ValidationSnafu,
};
...
pub async fn delete_vault(state: &AppState, id: &str) -> Result<()> {
    ...
    state.db.vaults.delete(id).await
}
